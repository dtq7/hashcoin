...
if(isset($_POST['key'])){
  ...
  if($_POST['key'] == 'getExistingData'){
    ...
    $sql = $con->query("SELECT id,username,TransactionId,unit,dates,addressed FROM buyRequest WHERE addressed=0 LIMIT $start, $limit ");
    if($sql->num_rows > 0){
        $response = "";
    ...
    if($_POST['key'] == 'deleteRow'){
        $con->query("UPDATE buyRequest SET addressed=1 WHERE id='$rowID'");
